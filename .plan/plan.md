...
- Scrape real user identities from Telegram
- Create users directly in mongodb (make sure I create no more than (a) 12,483, and (b) 10% of the total accounts)
- Add emails directly to Klaviyo
- Later, add valid-looking data to the user (make sure last played and created at are different)
- Ensure they don't get filtered out by a botting test

# Part B

Turn $NUGS across 10k accounts into $

- Do as much as possible to make this hard to trace

# CEX to use

Use a variety of no-KYC exchanges. If we're aiming for $5M, then 50 accounts is $100k. We don't let tokens linger, and we're moving $ in ~$10k amounts for safety. We swap to stablecoins where we can.
...
- NOT bybit, bitunix (kyc to deposit...)
  We can use one centralized no-kyc account to funnel the TON to the right location. We need ~15 decent identities to make this work (hoping to run that many through the 7 exchanges here, which would be 100 total)
...
- Create faked telegramInitData to allow us to authenticate an account with the bot

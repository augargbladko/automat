...
function createCreationGA(user: UserData, now: number): MpEvent[] {
  ...
  return {
    ...
    events: eventList.map((page) => {
      return {
        ...
        params: {
          session_id,
          engagement_time_msec: Math.floor(
            page === "home"
...
function getEventPlayParams(
  event: EventName,
  session_id: string,
  user: UserData
): MpEventParams & Record<string, string | number | undefined> {
  const params = {
    session_id,
    engagement_time_msec: Math.floor(
      event === EventName.login
        ? Math.random() * 1000 + 100
        : event === EventName.gameResults
          ? Math.random() * 2000 + 1000
          : event === EventName.ctaSeen || event === EventName.ctaTaken
            ? Math.random() * 300 + 100
            : event === EventName.questOpen
              ? Math.random() * 200 + 100
  ...
  switch (event) {
    case EventName.gameResults: {
      ...
      return {
        ...
        points: isPoints
          ? Math.floor(Math.random() * 500 + 100) * 1000 * multiplier
          : 0,
...
export async function sendGaForUser(
  user: UserData,

...
import {
  getJettonBalance,
  transferAllJettonsAndCloseWallet,
} from "./jettons.ts"
...
export async function TestTgInitData() {
  ...
  const { validatedData, telegramUser, message } =
...
export async function TestTonStuff() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    console.log(
      "Wallet address:",
      i,
      wallets[wallets.length - 1].address.toString({ bounceable: false })
    )
  }
